...
input double          Wave3MinStructureATR = 2.2;        // 识别 1-2 结构时所需的最小波幅，单位为 ATR 倍数
input double          TrendGapATR        = 0.18;         // 识别趋势状态时，快慢 EMA 间距至少达到多少 ATR
input double          RangeGapATR        = 0.05;         // 识别区间状态时，快慢 EMA 间距不超过多少 ATR
input double          RangeBreakoutATR   = 0.08;         // 区间反转信号突破确认时，对信号棒高低点增加的 ATR 缓冲

input group "==== 执行过滤 ===="
input bool            UseSessionFilter   = false;        // 是否启用交易时段过滤
input string          SessionStart       = "06:00";    // 允许开仓的开始时间，格式 HH:MM
input string          SessionEnd         = "23:00";    // 允许开仓的结束时间，格式 HH:MM
input int             MinBarsBetweenEntries = 8;        // 两次开仓之间最少间隔多少根 K 线
input int             MaxTradesPerDay    = 2;            // 每日最多允许开仓次数，0 表示不限制
...
input double          RiskPercent        = 0.5;          // 单笔交易风险占账户余额的百分比
...
input double          Wave3MinRatio      = 1.25;         // 第 3 浪相对第 1 浪的最小延伸倍数
...
input double          BreakoutBufferATR  = 0.10;         // 突破确认缓冲，按 ATR 的倍数加在突破位上
...
input double          MinATRPoints       = 140.0;        // 最小 ATR 点数，低于该波动阈值不交易
input double          StopBufferATR      = 0.25;         // 止损在结构低点/高点外额外预留的 ATR 缓冲
input double          RewardRisk         = 2.00;         // 固定止盈风险收益比
...
input double          TrailATRMultiplier = 1.20;         // 移动止损距离的 ATR 倍数
